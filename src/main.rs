...
mod zero_scaler;
pub(crate) use zero_scaler::{ActiveConnection, ZeroScaler};
...
async fn detect_upstreams(
  upstream_ip: Option<Ipv4Addr>,
  ports: Option<String>,
  service: Option<String>,
  namespace: &str,
) -> Result<Vec<(Ipv4Addr, (u16, String))>, kube::Error> {
  let upstreams = if let Some((ip, ports)) = upstream_ip.zip(ports) {
    ...
    let services: Vec<Service> = try_join_all(
      service
        .split(',')
    ...
    services
      ...
      .flat_map(|service| {
        ...
        let ports: Vec<(u16, String)> = service
          .spec
          .as_ref()
          .and_then(|s| {
...
async fn main() -> anyhow::Result<()> {
  ...
  let timeout: Duration =
    Duration::from_secs(env::var("TIMEOUT").map(|t| t.parse::<u64>().expect("TIMEOUT is not a number")).unwrap_or(600));
  ...
  let proxies = try_join_all(upstreams.into_iter().map(|(ip, (port, protocol))| match protocol.as_ref() {
    "tcp" => Either::Left(proxy::tcp(ip, port, scaler.as_ref(), proxy_type)),
    "udp" => {
