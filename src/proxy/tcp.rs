...
pub async fn tcp_proxy(host: Ipv4Addr, port: u16, scaler: &ZeroScaler, proxy_type: ProxyType) -> anyhow::Result<()> {
  ...
  listener_stream
    ...
    .try_for_each_concurrent(None, |downstream| async {
      ...
      let proxy = || async {
        ...
        let upstream = if replicas.available > 0 { Some(connect_to_upstream_server().await?) } else { None };

        let (downstream, mut upstream, active_connection) = match proxy_type {
          ProxyType::Minecraft => {
            let minecraft_favicon = env::var("MINECRAFT_FAVICON").ok();

            match middleware::minecraft::tcp(
              downstream,
              upstream,
              replicas.wanted,
              scaler,
              minecraft_favicon.as_deref(),
            )
            .await
            .context("Error in Minecraft middleware")?
            {
              Some((downstream, upstream, active_connection)) => (downstream, upstream, active_connection),
              None => return Ok(()),
            }
          },
          _ => {
        ...
        let upstream =
          if let Some(upstream) = upstream.take() { upstream } else { connect_to_upstream_server().await? };
